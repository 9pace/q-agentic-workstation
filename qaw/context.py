...
class CodebaseAnalyzer:
    ...
    def _detect_databases(self) -> Set[str]:
        ...
        for db_name, indicators in self.DATABASE_INDICATORS.items():
            for indicator in indicators:
                if self._search_in_files(indicator, ['package.json', 'requirements.txt', 'Pipfile']):
    ...
    def _detect_naming_conventions(self) -> List[str]:
        ...
        sample_files = list(self.project_root.rglob('*.py'))[:10] + \
    ...
    def _generate_constraints(self, tech_stack: TechStack, patterns: ProjectPatterns) -> List[str]:
    ...
    def _should_ignore(self, path: Path) -> bool:
        ...
        ignore_patterns = [
            'node_modules', '.git', '__pycache__', '.qaw',
    ...
    def _search_in_code(self, patterns: List[str]) -> bool:
        ...
        for ext in extensions:
            for file_path in list(self.project_root.rglob(f'*{ext}'))[:20]:  # Sample 20 files
                if self._should_ignore(file_path):
                    continue
...
"""
        ...
        if 'TypeScript' in context.tech_stack.languages or 'JavaScript' in context.tech_stack.languages:
            content += """
