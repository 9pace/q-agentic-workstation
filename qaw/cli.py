...
@click.option(
    "--name", default=None, help="Custom agent name (auto-generated if not provided)"
)
@click.option(
    "--trust-all-tools", is_flag=True, help="Trust all tools without confirmation"
)
@click.option(
    "--autopilot", is_flag=True, help="Enable autopilot mode (no timeout, keep-alive)"
)
...
def submit(
    ...
    try:
        ...
        if autopilot:
            click.secho(
                "🤖 Autopilot mode enabled - agent will run until completion", fg="cyan"
            )
...
@click.option(
    "--all", "show_all", is_flag=True, help="Show all agents (not just recent)"
)
@click.option("--verbose", "-v", is_flag=True, help="Verbose output")
        ...
        if agent_id:
            ...
            if agents:
                ...
                for agent in agents:
                    ...
                    click.echo(
                        f"{agent_id_str:<15} {agent.status.value:<12} {agent.agent_name[:18]:<20} {duration:<12} {task_preview}"
                    )
            else:
                click.echo(
...
def stop(agent_id: str, force: bool):
    ...
    try:
        ...
        if executor.stop_agent(agent_id, force=force):
            ...
            click.secho(
                f"Agent {agent_id} is not running or already stopped", fg="yellow"
            )
                    ...
                    try:
                        ...
                        if user_input.lower() in ["execute", "exec", "run"]:
                            if not session.current_plan:
                                click.secho(
                                    "⚠️  No plan to execute. Send a request first.",
                                    fg="yellow",
                                )
                                continue
                            ...
                            def progress_callback(increment):
                                ...
                                click.echo(
                                    f"{emoji} {increment.description} [{status}]"
                                )

                            success = await session.execute_current_plan(
                                progress_callback
                            )
...
@cli.command()
def dashboard():
    """Launch interactive hybrid dashboard with Manager + Agent Monitor."""
    try:
        from .dashboard_hybrid import run_hybrid_dashboard
        ...
        if not (workspace / "state").exists():
            click.secho(
                "Workspace not initialized. Run 'qaw init' first.",
                fg="yellow",
                err=True,
            )
            sys.exit(1)

        click.secho("🚀 Launching Hybrid Dashboard...", fg="cyan", bold=True)
        click.echo("Two interfaces in one:")
        click.echo("  • Manager Tab: Natural language task planning")
        click.echo("  • Monitor Tab: Interactive agent table with keyboard control")
        click.echo("Press '1' for Manager, '2' for Monitor, 'q' to quit, '?' for help")
        click.echo()

        run_hybrid_dashboard(workspace)

    except ImportError:
        click.secho(
            "Dashboard requires 'textual'. Install with: pip install textual",
            fg="red",
            err=True,
        )
        sys.exit(1)
...
@cli.command()
def app():
    """Launch the Manager-first app (alias for dashboard)."""
    try:
        ...
        click.secho("🚀 Welcome to Q Agentic Workstation!", fg="cyan", bold=True)
        click.echo("\nManager-First Interface:")
        click.echo("  • Describe tasks in natural language")
        click.echo("  • Manager breaks them into increments")
        click.echo("  • Watch agents execute in real-time")
        click.echo()
        ...
        click.secho(
            "App requires 'textual'. Install with: pip install textual",
            fg="red",
            err=True,
        )
        sys.exit(1)
